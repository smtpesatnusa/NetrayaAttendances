...
namespace SMTAttendance
{
    public class Helper
    {
        ...
        public void ApplyMaterialSkin(MaterialForm form)
        {
        ...
        public void displayCmb(string sql, string column, ComboBox comboBox)
        {
            try
            {
                using (MySqlDataAdapter adpt = new MySqlDataAdapter(sql, connectionDB.connection))
                {
                    ...
                    if (dt.Rows.Count > 0)
                    {
                        ...
                        {
                            comboBox.Items.Add(dt.Rows[i][column].ToString());
                        }
                    }
                }
                comboBox.SelectedIndex = -1;
        ...
        public void addCmbList(string sql, string display, string value, ComboBox comboBox)
        {
            try
            {
                using (MySqlDataAdapter adpt = new MySqlDataAdapter(sql, connectionDB.connection))
                {
                    ...
                    if (dt.Rows.Count > 0)
                    {
                        comboBox.DisplayMember = display;
                        comboBox.ValueMember = value;
                        comboBox.Items.Add(new { display, value });
