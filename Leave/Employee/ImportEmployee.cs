        ...
        private void bgWorker_DoWork(object sender, System.ComponentModel.DoWorkEventArgs e)
        {
            try
            {
                ...
                for (int i = 0; i < dataGridViewEmployee.Rows.Count; i++)
                {
                    ...
                    // insert schedule if employee is normal shift
                    if (shift == "Normal" || shift == "normal")
                    {
                        ...
                        string Query = "INSERT INTO tbl_schedule (badgeId, shiftId, dateShift, createDate, createBy) " +
                            "VALUES ('"+badgeId+"', (SELECT id FROM tbl_mastershiftdetail WHERE category = 'Normal'), '1601-01-01', '"+ createDate + "','" + createBy + "')";
        ...
        private void browseEmployee_Click(object sender, EventArgs e)
        {
            ...
            if (openFileDialogEmployee.ShowDialog() == DialogResult.OK)
            {
                ...
                try
                {
                    ...
                    {
                        CloseProgress();
                        MessageBox.Show(this, "There is any duplicate Data in file, not allow import duplicate badge id  or rfid no data please check!!"+countduplicatebadge+"-"+countduplicaterfid, "Warning", MessageBoxButtons.OK, MessageBoxIcon.Error); //custom messageBox to show error 
